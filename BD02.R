...
df <- data.frame(bang = date_1$Province_State, Deaths = date_1$Deaths)
df

bp<- ggplot(df, aes(x="", y=Deaths, fill=bang))+
  geom_bar(width = 1, stat = "identity")
bp + labs(title="Bieu do the hien so ca tu vong do Covid cua cac bang thuoc My vao 01-01-2021 ")
